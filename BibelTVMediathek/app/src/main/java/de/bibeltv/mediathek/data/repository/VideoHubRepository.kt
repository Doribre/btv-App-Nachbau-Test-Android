...
import de.bibeltv.mediathek.graphql.Bre_testapp_GenreVideosQuery
import de.bibeltv.mediathek.graphql.Bre_testapp_GenresQuery
import de.bibeltv.mediathek.graphql.Bre_testapp_LiveStreamByIdQuery
...
class VideoHubRepository @Inject constructor(
    ...
    private suspend fun <T> withRetry(attempts: Int = 4, block: suspend () -> T): T {
        ...
        repeat(attempts) { attempt ->
            try {
                ...
                lastError = e
                // Backoff: 400ms, 800ms, 1200ms zwischen den Versuchen.
    ...
    suspend fun browseNewest(skip: Int, take: Int): List<VideoItem> = withRetry {
        apollo.query(Bre_testapp_NewestVideosQuery(take = take, skip = skip, now = nowIso())).execute()
            .dataOrThrow().videos.map { it.videoCard.toDomain() }
    }

    suspend fun browseByGenre(genreId: Int, skip: Int, take: Int): List<VideoItem> = withRetry {
        apollo.query(Bre_testapp_GenreVideosQuery(genreId = genreId, take = take, skip = skip, now = nowIso())).execute()
            .dataOrThrow().genre?.videos?.map { it.videoCard.toDomain() } ?: emptyList()
    }

    suspend fun searchVideos(query: String, skip: Int, take: Int): List<VideoItem> = withRetry {
        apollo.query(Bre_testapp_SearchVideosQuery(q = query, take = take, skip = skip, now = nowIso())).execute()
            .dataOrThrow().videos.map { it.videoCard.toDomain() }
    }

    suspend fun videoDetail(crn: String): VideoDetailModel? = withRetry {
        apollo.query(Bre_testapp_VideoDetailQuery(crn = crn)).execute()
            .dataOrThrow().videos.firstOrNull()?.toDetail()
    }

    suspend fun seriesDetail(id: Int): SeriesDetailModel? = withRetry {
        apollo.query(Bre_testapp_SeriesDetailQuery(id = id, now = nowIso())).execute()
            .dataOrThrow().serie?.toDetail()
    }

    suspend fun videoPlayout(crn: String): PlayoutSource? = withRetry {
        apollo.query(Bre_testapp_VideoPlayoutQuery(crn = crn)).execute()
            .dataOrThrow().videos.firstOrNull()?.toPlayoutSource()
    }

    suspend fun liveSource(id: Int): PlayoutSource? = withRetry {
        val ls = apollo.query(Bre_testapp_LiveStreamByIdQuery(id = id)).execute().dataOrThrow().liveStream ?: return@withRetry null
